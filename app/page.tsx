      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <Card className="w-full max-w-md shadow-lg border-gray-200 bg-white">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl text-black font-bold">Iniciar Sesión</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setTipoUsuario("candidato")}
                    ...
                    <span className={`text-sm ${tipoUsuario === "candidato" ? "text-black font-bold" : "text-gray-600"}`}>Candidato</span>
                  </button>
                  <button type="button" onClick={() => setTipoUsuario("empresa")}
                    ...
                    <span className={`text-sm ${tipoUsuario === "empresa" ? "text-black font-bold" : "text-gray-600"}`}>Empresa</span>
                  </button>
                </div>
              </div>
              ...
              <button type="submit" disabled={isLoading}
                ...
                {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Conectando...</> : "Iniciar Sesión"}
              </button>
            </form>
          </CardContent>
          ...
          <CardFooter className="flex flex-col space-y-3 pt-4 border-t border-gray-100">
            <div className="text-sm text-gray-600 text-center w-full">
              ...
              <Link href="/register" className="text-[#5FD3BC] hover:underline font-bold">Regístrate aquí</Link>
            </div>
            <div className="text-sm text-gray-600 text-center w-full">
              ¿Olvidaste tu contraseña?{" "}
              <Link href="/forgot-password" className="text-[#5FD3BC] hover:underline font-bold">Recupera tu cuenta aquí</Link>
            </div>
          </CardFooter>
        </Card>
      </div>
